...
class Day7(private val input: List<String>) {
    ...
    data class Hand(val hand: String, val bid: Int, val hasJokers: Boolean) : Comparable<Hand> {
        ...
        init {
            ...
            if (hasJokers) {
                ...
                if (jokers in 1..4) {
                    val max = map.entries.filterNot { it.key == 'J' }.maxByOrNull { it.value }!!.key
                    map.remove('J')
...
fun main() {
    val fileInput: List<String> = File("src/main/kotlin/inputs/Day7.txt").readLines()
    println(Day7(fileInput).solution1())
    println(Day7(fileInput).solution2())
}