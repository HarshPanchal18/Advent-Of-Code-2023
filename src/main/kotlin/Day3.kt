...
class Day3(private val input: List<String>) {
    fun solution1() {
        ...
        for (symbol in symbols) {
            val matches =
                numbers.filter { symbol.y >= it.y - 1 && symbol.y <= it.y + 1 && symbol.x >= it.x - 1 && symbol.x <= it.xEnd + 1 }
    ...
    fun solution2() {
        ...
        for (symbol in symbols.filter { it.value == '*' }) {
            val matches =
                numbers.filter {
                    symbol.y >= it.y - 1 && symbol.y <= it.y + 1 &&
                            symbol.x >= it.x - 1 && symbol.x <= it.xEnd + 1
                }
...
fun main() {
    Day3(File("src/main/kotlin/inputs/Day3.txt").readLines()).solution1()
    Day3(File("src/main/kotlin/inputs/Day3.txt").readLines()).solution2()
}