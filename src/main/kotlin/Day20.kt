...
class Day20(val input: List<String>) {
    ...
    private val modules =
        input.map { Module.from(it.split(" -> ").first()) }.associateBy { it.name } +
    ...
    fun solution2(): Long {
        ...
        while (monitored.values.any { it == 0L }) {
            i++
            var current = listOf(modules.getValue(key = "broadcaster") to false)
    ...
    private sealed class Module {
        ...
        data class FlipFlop(override val name: String, var state: Boolean) : Module() {
            ...
            override fun output(receivedPulse: Boolean): Boolean? {
                return if (receivedPulse) null else state
            }
        }
        ...
        data class Conjunction(override val name: String, val state: MutableMap<String, Boolean>) : Module() {
            override fun receive(source: String, pulse: Boolean) {
                state[source] = pulse
            }
        ...
        companion object {
            fun from(str: String): Module {
                return when (str.first()) {
                    ...
                    '&' -> Conjunction(name = str.drop(1), state = mutableMapOf())
                    else -> Broadcaster(str)
                }
            }
        }
    }
...
fun main() {
    val fileInput = File("src/main/kotlin/inputs/Day20.txt").readLines()
    println(Day20(fileInput).solution1())
    println(Day20(fileInput).solution2())
}