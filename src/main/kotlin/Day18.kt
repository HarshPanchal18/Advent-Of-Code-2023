...
class Day18(val input: List<String>) {
    ...
    fun solution1(): Long = calculateArea { line ->
        line.split(' ').let { parts ->
            directionsMap.getValue(parts.first().first()) to parts[1].toInt()
        }
    }

    @OptIn(ExperimentalStdlibApi::class)
    fun solution2(): Long = calculateArea { line ->
        colorRegex.find(line)?.let { it.groupValues[2].toInt() to it.groupValues[1].hexToInt() }
...
fun main() {
    val fileInput = File("src/main/kotlin/inputs/Day18.txt").readLines()
    println(Day18(fileInput).solution1())
    println(Day18(fileInput).solution2())
}