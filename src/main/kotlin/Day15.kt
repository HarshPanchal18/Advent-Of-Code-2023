...
class Day15(val input: String) {
    ...
    fun solution2(): Int {
        ...
        steps.forEach { step ->
            val (lense, value) = step.split('-', '=')
            val slots = boxes[lense.hash()]
            val index = slots.indexOfFirst { (label, _) -> label == lense }

            when {
                '-' in step && index >= 0 -> slots.removeAt(index)
                '=' in step && index >= 0 -> slots[index] = lense to value
                '=' in step -> slots.add(lense to value)
            }
        }

        return boxes.withIndex().sumOf { (num, slots) ->
            slots.withIndex().sumOf { (position, slot) -> (num + 1) * (position + 1) * slot.second.toInt() }
        }
    }
}

fun main() {
    val fileInput = File("src/main/kotlin/inputs/Day15.txt").readText()
    println(Day15(fileInput).solution1())
    println(Day15(fileInput).solution2())
}