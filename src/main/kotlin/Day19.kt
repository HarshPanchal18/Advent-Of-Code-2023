...
class Day19(input: String) {
    ...
    private fun String.toExpr(): Expr = when {
        "A" == this -> Expr.Accept
        "R" == this -> Expr.Reject
        ">" in this -> split('>', ':').let { (name, value, next) ->
            Expr.Compare(name, '>', value.toLong(), next.toExpr())
        }
    ...
    fun solution1(): Long {
        return environments.sumOf { env ->
            fun Expr.Compare.predicate(): Boolean = when (op) {
            ...
            do {
                when (val expr = expressions.removeFirst()) {
                    is Expr.Accept -> accept = true

                    is Expr.Compare -> if (expr.predicate()) {
                        expressions.clear()
...
fun main() {
    val fileInput = File("src/main/kotlin/inputs/Day19.txt").readText()
    println(Day19(fileInput).solution1())
}