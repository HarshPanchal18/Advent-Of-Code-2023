...
class Day9(private val input: List<String>) {
    ...
    fun solution1() {
        ...
        val sum = sequences.sumOf { resolveSequence(it) }
        println(sum)
    }
    ...
    fun solution2() {
        val sequences = input.map { line -> line.split(' ').map { Integer.parseInt(it) } }
        val sum = sequences.sumOf { resolveSequence1(it) }
        println(sum)
    }

    private fun resolveSequence1(sequence: List<Int>): Int {
        val comparisons = sequence.windowed(2, 1).map { it[1] - it[0] }
        if (comparisons.all { it == comparisons[0] })
            return sequence.first() - comparisons[0]

        return sequence.first() - resolveSequence1(comparisons)
    }
}

fun main() {
    val fileInput: List<String> = File("src/main/kotlin/inputs/Day9.txt").readLines()
    Day9(fileInput).solution1()
    Day9(fileInput).solution2()
}