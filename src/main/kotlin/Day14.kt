...
class Day14(val input: List<String>) {
    ...
    fun solution2(): Int {
        ...
        while (step < 1_000_000_000) {
            repeat(4) {
                tiltUp()
                deck =
                    deck.rotate() // You could also use tiltLeft(), tiltDown(), tiltRight() could be used instead of rotate()
            }
    ...
    private val Array<CharArray>.score
        get() = indices.fold(0) { acc, y ->
...
fun main() {
    val fileInput = File("src/main/kotlin/inputs/Day14.txt").readLines()
    println(Day14(fileInput).solution1())
    println(Day14(fileInput).solution2())
}