...
class Day8(input: List<String>) {
    ...
    private val nodes =
        input.drop(2).associate { it.substringBefore(" =") to it.substringAfter("(").substringBefore(")").split(", ") }
    ...
    fun solution2(): Long {
        ...
        val steps = beginning.map { node -> steps(node, repeat = { it.last() != 'Z' }) }

        return steps.lcm()
    }

    private fun List<Long>.lcm(): Long {
        return reduce { a, b -> lcm(a, b) }
    }
...
fun main() {
    val fileInput: List<String> = File("src/main/kotlin/inputs/Day8.txt").readLines()
    println(Day8(fileInput).solution1())
    println(Day8(fileInput).solution2())
}