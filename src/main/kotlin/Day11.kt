...
class Day11(val input: List<String>) {
    ...
    fun solution1() {
        println(sumOfDistance(expandBy = 2))
    }

    fun solution2() {
        println(sumOfDistance(expandBy = 10_00_000))
    }

    private fun sumOfDistance(expandBy: Int): Long {
        ...
        val sumColumns =
            input.first().indices
                ...
                .runningFold(0) { prev, column ->
                    if (column.any { it == '#' }) prev else prev + 1
                }
        ...
        val galaxies = input.flatMapIndexed { rowIndex, row ->
            row.mapIndexedNotNull { columnIndex, column ->
                if (column == '#') {
                    Point2D(
                        x = columnIndex + sumColumns[columnIndex] * (expandBy - 1),
                        y = rowIndex + sumRows[rowIndex] * (expandBy - 1)
                    )
                } else null
            }
        }
...
fun main() {
    val fileInput: List<String> = File("src/main/kotlin/inputs/Day11.txt").readLines()
    Day11(fileInput).solution1()
    Day11(fileInput).solution2()
}